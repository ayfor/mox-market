...
function SessionActions() {
  ...
  const handleExport = () => {
    ...
    a.href = url;
    a.download = `mox-market-watchlist-${new Date().toISOString().slice(0, 10)}.json`;
    a.click();
    ...
    input.onchange = async (e) => {
      ...
      try {
        ...
        if (Array.isArray(imported)) {
          ...
          store.clearAll();
          // Set cards directly via the store
          useWatchlistStore.setState({ cards: imported });
        }
      } catch {
  ...
  return (
    <div className="mt-3 flex gap-2">
      <button
        ...
        className="flex-1 rounded-md px-2 py-1 text-xs font-medium text-gray-500 hover:bg-gray-950/5 hover:text-gray-700 dark:text-gray-400 dark:hover:bg-white/5 dark:hover:text-gray-300"
      >
        Export
      </button>
      <button
